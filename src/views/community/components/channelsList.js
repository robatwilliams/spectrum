...
const ChatTab = ({ location, community, currentUser }) => {
  const match = useMatch(`/${community.slug}`);
...
const Component = (props: Props) => {
  ...
  const communityMatch = useMatch('/:communitySlug');
  const channelMatch = useMatch('/:communitySlug/:channelSlug');
      ...
      <React.Fragment>
        ...
        <List data-cy="channel-list">
          ...
          <Link to={`/${community.slug}?tab=posts`}>
            <Row
              isActive={
                !!communityMatch && location.search.indexOf('tab=posts') > -1
              }
            >
              <Content>
                <Label>
          ...
          {sortedChannels.map(channel => {
            if (!channel) return null;
            const isActive =
              channelMatch && channelMatch.params.channelSlug === channel.slug;
            return (
