...
const BrandedLogin = (props: Props) => {
  ...
  React.useEffect(() => {
    if (props.data.community && messageValue === null) {
      setMessageValue(props.data.community.brandedLogin.message);
    }
  }, [props.data.community, messageValue]);
  ...
  const saveCustomMessage = e => {
    ...
    return props
      .saveBrandedLoginSettings({
        ...
        id: props.data.community.id,
      })
      .then(() => {
        setMessageLengthError(false);
        setIsLoading(false);
        return props.dispatch(addToastWithTimeout('success', 'Saved!'));
      })
      .catch(err => {
        setMessageLengthError(false);
        setIsLoading(false);
